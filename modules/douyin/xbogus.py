"""
XBogus signature algorithm for Douyin API requests.

Generates X-Bogus parameter required by Douyin's web API.
Uses MD5 hashing and RC4 encryption bound to User-Agent.
"""
...
class XBogus:
    """XBogus signature generator for Douyin API."""
    ...
    def build(self, url: str) -> Tuple[str, str, str]:
        ...
        Args:
            url: The API URL with query parameters.

        Returns:
            Tuple of (signed_url, xbogus_value, user_agent).
        """
...
def generate_x_bogus(url: str, user_agent: Optional[str] = None) -> Tuple[str, str, str]:
    """
    Convenience function to generate X-Bogus for a URL.

    Args:
        url: API URL with query parameters.
